...
const db=JSON.parse(fs.readFileSync(path.join(__dirname,'spots_top100.json'),'utf8'));
const spots=db.spots;
const vecPath=path.join(__dirname,'spot_vectors.f32');
if(!fs.existsSync(vecPath)){
  console.error('spot_vectors.f32가 없습니다. 먼저 node build_embeddings.mjs 를 실행하세요.');process.exit(1);
}
const raw=fs.readFileSync(vecPath);
if(raw.length!==spots.length*DIM*4){console.error('벡터 파일 크기가 DB와 맞지 않습니다. build_embeddings.mjs를 다시 완료하세요.');process.exit(1)}
const vectors=new Float32Array(raw.buffer,raw.byteOffset,raw.length/4);
const countPath=path.join(__dirname,'recommend_counts.json');
let counts={}; try{counts=JSON.parse(fs.readFileSync(countPath,'utf8'))}catch{}
...
async function generate(category,wish,cands,attempt=0){
  ...
  try{
        ...
        generationConfig:{
          ...
          responseSchema:{type:'OBJECT',properties:{
            analysis:{type:'OBJECT',properties:{
              subject:{type:'STRING'},situation:{type:'STRING'},desired_change:{type:'STRING'}
            },required:['subject','situation','desired_change']},
            letter:{type:'STRING'},
            picks:{type:'ARRAY',minItems:3,maxItems:3,items:{type:'OBJECT',properties:{
              id:{type:'INTEGER'},
              connection_type:{type:'STRING'},
              place_feature:{type:'STRING'},
              wish_link:{type:'STRING'},
              catch:{type:'STRING'}
            },required:['id','connection_type','place_feature','wish_link','catch']}}
          },required:['analysis','letter','picks']}
        }
      })
    },8500);
    const responseText=await r.text();
    let d={}; try{d=JSON.parse(responseText||'{}')}catch{}
    if(!r.ok){
      if((r.status===429||r.status>=500)&&attempt<1){await sleep(220);return generate(category,wish,cands,attempt+1)}
    ...
    try{
      ...
      if(attempt<1 && /Unexpected end|unterminated|JSON/i.test(String(parseError.message))){
        await sleep(120);
        return generate(category,wish,cands,attempt+1);
      }
      throw parseError;
    }
  }catch(e){
    if(attempt<1 && /fetch|network|socket|timeout|abort/i.test(String(e.message))){await sleep(220);return generate(category,wish,cands,attempt+1)}
    throw e;
  }
}
...
function logUsage(category,wish,usage,candidateCount){
  ...
  fs.appendFileSync(path.join(__dirname,'gemini_usage.jsonl'),JSON.stringify(row)+'\n');
}
async function recommend(category,wish){
  ...
  const finalPool=(safePool.length>=18?safePool:cands).slice(0,32);
  if(finalPool.length<3) throw new Error('추천 후보가 부족합니다.');

  let result=null;
  try{result=await generate(category,wish,finalPool);}catch(e){console.warn('Gemini 생성 실패, 로컬 복구:',e.message);}
  ...
  // API 자체가 실패하거나 구조가 깨졌을 때만 로컬 비상 복구. 정상 상황에서는 절대 사용하지 않는다.
  if(picks.length<3){
    const existingNames=new Set(picks.map(p=>p['명소명']));
    const recovered=pickRecovery(finalPool,wish,category,picks);
    for(const p of recovered){
      if(!existingNames.has(p['명소명'])){picks.push(p);existingNames.add(p['명소명']);}
      if(picks.length===3) break;
    }
  }
  if(picks.length<3) throw new Error('추천 3곳을 완성하지 못했습니다.');

  let letter=result?.letter?repairKorean(cleanKoText(result.letter,260)):smartLetter(category,wish);
  if(!letter||hasBadKorean(letter)||genericLetter(letter)) letter=smartLetter(category,wish);

  // 카피 중복 방지: 동일 문장을 그대로 반복하는 경우에만 비상 교체한다.
  const seenCatch=new Set();
  for(let i=0;i<picks.length;i++){
    let ct=repairKorean(cleanKoText(picks[i].catch,96));
    if(seenCatch.has(ct)){
      const c=finalPool.find(x=>x.name===picks[i]['명소명']);
      if(c) ct=smartCatch(c,wish,category);
    }
    picks[i].catch=ct;
    seenCatch.add(ct);
...
const server=http.createServer(async(req,res)=>{try{
  if(req.method==='POST'&&req.url==='/api/recommend'){
    let raw='';for await(const ch of req)raw+=ch;const b=JSON.parse(raw||'{}');
    if(!b.category||!b.wish)return send(res,400,{error:'category/wish 필요'});
    const result=await recommend(b.category,b.wish);return send(res,200,result);
  }
  let f=req.url==='/'?'/test_gemini_36_flash.html':req.url.split('?')[0];const full=path.join(__dirname,path.normalize(f));
  if(!full.startsWith(__dirname)||!fs.existsSync(full)||fs.statSync(full).isDirectory())return send(res,404,'Not found','text/plain; charset=utf-8');
  const ext=path.extname(full);const types={'.html':'text/html; charset=utf-8','.json':'application/json; charset=utf-8'};return send(res,200,fs.readFileSync(full),types[ext]||'application/octet-stream');
}catch(e){
  console.error(e);
  return send(res,503,{error:'달님이 답장을 다시 정리하고 있어. 잠시 뒤 한 번만 다시 눌러줘!'});
}});
server.listen(8787,()=>console.log('FULL DB 테스트: http://localhost:8787/test_gemini_36_flash.html'));
